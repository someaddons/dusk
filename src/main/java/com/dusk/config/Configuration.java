...
public class Configuration
{
    ...
    public void load()
    {
        final Path configPath = FabricLoader.getInstance().getConfigDir().normalize().resolve(Dusk.MODID + ".json");
        ...
        if (!config.exists())
        {
            Dusk.LOGGER.warn("Config for dynamic view not found, recreating default");
            try
            {
            ...
            {
                Dusk.LOGGER.error("Could not write config to:" + configPath, e);
            }
        }
        else
        {
            ...
            {
                Dusk.LOGGER.error("Could not read config from:" + configPath, e);
            }
        }
    }
